...
def test_registr_by_valid_data_(selenium):
    """TК-15 Регистрация пользователя с валидными данными:
...
def test_registr_and_redir_auth(selenium):
    """TК-17 Регистрация пользователя по email, который уже был использован ранее для регистрации
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    """TК-18 Проверка поля ввода "Имя" формы «Регистрация» валидными данными:
...
def test_first_name_by_invalid_data(selenium, invalid_first_name):
    """TК-19 Проверка поля ввода "Имя" формы «Регистрация» невалидными данными:
    пустое значение;
...
def test_last_name_by_valid_data(selenium, valid_password):
    """TК-20 Проверка поля ввода "Пароль" формы «Регистрация» валидными данными:
...
def test_passw_registration_confirm_valid_data(selenium):
    """TК-21 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными
    (пароли совпадают)."""
    page_reg = RegistrPage(selenium)
...
def test_passw_registration_confirm_invalid_data(selenium):
    """TК-22 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными
    (пароли не совпадают)."""
    page_reg = RegistrPage(selenium)
    page_reg.passw_registration.send_keys(Settings.passw1)
    page_reg.passw_registration.clear()
